...
public:
    // Constructor
    Customer(const std::string &name, int accountNumber) : _name_(name), _account_(accountNumber) {}
    ...
    void deposit(double amount) {
        ...
        std::cout << "Depositing $" << amount << " into " << _name_ << "'s account" << std::endl;
    }

    // Define a function to withdraw money from the customer's account
    void withdraw(double amount) {
        _account_.withdraw(amount);
        std::cout << "Withdrawing $" << amount << " from " << _name_ << "'s account" << std::endl;
    }
};
    ...
    void displayCustomerInfo(int accountNumber) const {
        for (const auto &customer : _customers_) {
            if (customer->getAccountNumber() == accountNumber) {
                ...
                std::cout << "Account Balance: $" << customer->getAccountBalance() << std::endl;
                std::cout << std::endl;
                return;
            }
        }
        std::cout << "Customer with Account Number " << accountNumber << " not found." << std::endl;
        std::cout << std::endl;
    }
};
