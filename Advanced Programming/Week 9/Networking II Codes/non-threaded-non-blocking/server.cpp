...
int main() 
{
    ...
    // Set the server socket to non-blocking mode
        ...
        {
            std::cerr << "Failed to set server " << 
            "socket to non-blocking mode" << std::endl;
            return 1;
        }
    #else
        // Set the server socket to non-blocking mode using ioctl
        int nonBlockingMode = 1;
        if (ioctl(serverSocket, FIONBIO, &nonBlockingMode) == -1) 
        {
            std::cerr << "Failed to set server " << 
            "socket to non-blocking mode" << std::endl;
            return 1;
    ...
    while (true) {
        ...
        if (select(maxSocket + 1, &readfds, 
        nullptr, nullptr, &timeout) == -1) {
